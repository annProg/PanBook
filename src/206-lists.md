...
```{#lst:block_content_inlist_exception .markdown caption="区块内容作为列表项例外情况"}
*     code
...
```{#lst:fancy_lists .markdown caption="# 代替数字作为有序列表标记"}
#. one
#. two
```
...
```{#lst:definition_lists .markdown caption="定义列表"}
Term 1
...
编号范例可以加上标签，并且在文件的其他地方作参照 [@lst:example_lists_ref]。
